...
public class WeiBo {
    ...
    //微博内容表的表名
    private final byte[] TABLE_CONTENT = Bytes.toBytes("weibo:content");
    //用户关系表的表名
    private final byte[] TABLE_RELATIONS = Bytes.toBytes("weibo:relations");
    //微博收件箱表的表名
    private final byte[] TABLE_RECEIVE_CONTENT_EMAIL = Bytes.toBytes("weibo:receive_content_email");
    ...
    WeiBo(){
        try{
            ...
            this.conf.set("hbase.zookeeper.quorum", "150.158.174.69,121.4.205.232,111.229.176.181");
